from chess.helpers import Move, validSquare
    ...
    def exploreMoves(self):
        ...
        for square in self.positions[not white]:
            ...
            if pieceType in ["k", "-"]:
                continue
            if piece == "P":
                if validSquare((square[0] + 1, square[1] + 1)):  # For captures
                        ...
                        if(ChessBoard.validMove(move, self.boardState.copy(), white, kingSquare)):
                            if(square[0] == 6):
                                for promType in ["Q", "N", "B", "R"]:
                                    out.append(Move(piece, square, (square[0] + 1, square[1] + 1), None, promType, isDirectCapture = True))
                            else:
                                out.append(move)
                if validSquare((square[0] + 1, square[1] - 1)):
                    existingPiece = self.boardState[square[0] + 1][square[1] - 1]
                    if (existingPiece != "-" and existingPiece.lower() == existingPiece):
                        move = Move(piece, square, (square[0] + 1, square[1] - 1), None, None, isDirectCapture = True)
                        if(ChessBoard.validMove(move, self.boardState.copy(), white, kingSquare)):
                ...
                if self.boardState[square[0] + 1][square[1]] == "-" :
                    ...
                    if(ChessBoard.validMove(move, self.boardState.copy(), white, kingSquare)):  # Normal one square moves
                        if(square[0] == 6): # In case of promotion
                            for promType in ["Q", "N", "B", "R"]:
                                out.append(Move(piece, square, (square[0] + 1, square[1]), None, promType))
                        else:
                            out.append(move)
                    if square[0] == 1 and self.boardState[square[0] + 2][square[1]] == "-": # Double step
                        ...
                        if(ChessBoard.validMove(move, self.boardState.copy(), white, kingSquare)):
                            out.append(move)
                if square[0] == 4 and not(self.enPassant is None) and abs(self.enPassant - square[1]) == 1: # En passant
                ...
                if self.boardState[square[0] - 1][square[1]] == "-" :
                    ...
                    if square[0] == 6 and self.boardState[square[0] - 2][square[1]] == "-":
                        ...
                        if(ChessBoard.validMove(move, self.boardState.copy(), white, kingSquare)):
                            out.append(move)
                if square[0] == 3 and not(self.enPassant is None) and abs(self.enPassant - square[1]) == 1:
                    move = Move(piece, square, (square[0] - 1, self.enPassant), enPassant= True)
                    if(ChessBoard.validMove(move, self.boardState.copy(), white, kingSquare)):
                        out.append(move)
            if pieceType in ["r", "q"]:
                for i in  range(1, square[1] + 1):
                    ...
                    if(squarePiece == "-" ):
                        move = Move(piece, square, newSquare)
                        if(ChessBoard.validMove(move, self.boardState.copy(), white, kingSquare)):
                            out.append(move)
                        continue
    ...
    def makeMove(self, move: Move) -> ChessBoard: 
        ...
        if(move.enPassant):
            ...
            if move.castleType == "K":
                ...
                out.castling[2] = False
                out.castling[3] = False
        elif not (move.promotion is None):
            out.boardState[move.newSquare[0]] = out.boardState[move.newSquare[0]][:move.newSquare[1]] + move.promotion + out.boardState[move.newSquare[0]][move.newSquare[1] + 1:]
        ...
        if (out.castling != self.castling or move.isDirectCapture or move.piece.lower() == "p"):
            out.halfMoves = 0
            out.threeMoveDict = {}
        else:
            out.halfMoves = self.halfMoves + 1
        ...
        if out.castling[3]:
            castleString += "q"
        out.threeMoveString =''.join(out.boardState) + castleString + ("w" if out.whitesTurn else "b") + ( "8" if out.enPassant is None else str(self.enPassant))
        out.possibleMoves = out.exploreMoves()
        return out
